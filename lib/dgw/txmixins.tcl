...
#'     - [dgw::txfontsize](#txfontsize) - change fontsize using Ctrl-Plus or Ctrl-Minus
...
#'     - [dgw::txtabspace](#txtabspace) - change the TAB key to four spaces insertion
#'  - [EXAMPLE](#example)
...
package provide dgw::txmixins 0.3.0
...
#' 
#' <a name="txfontsize">**dgw::txfontsize**</a> *[tk::text pathName] ? option value ...?*
#' 
#' Adds the Control-Plus and Control-Minus fontsize changing options to text widget given with _pathNName_.
#' All options and methods are delegated to the standard text widget.
#'
#' The following options are available:
#'
#' > - *-increasekey* - the key sequence to increase the fontsize, default: <Control-plus>
#'   - *-decreasefontkey* - the key sequence to increase the fontsize, default: <Control-minus>
#' 
#' The widget has the following method:
#' 
#' *pathName changeFontsize x* - where *x* is an integer specifiying the size to increase or decrease the font.
#' 
#' Example:
#' 
#' > ```
#' # demo: txfontsize
#' dgw::txfontsize [tk::text .txt]
#' foreach col [list A B C] { 
#'    .txt insert  end "# Header $col\n\nSome text\n\n"
#' }
#' .txt insert end "Press Ctrl-Plus or Ctrl-Minus to change the fontsize!"
#' bind .txt <Enter> { focus -force .txt }
#' pack .txt -side top -fill both -expand true
#' > ```
#' 
# widget adaptor which allows easy change of fontsizes Ctrl-+ / Ctrl--

snit::widgetadaptor dgw::txfontsize {
    option -increasekey Control-plus
    option -decreasekey Control-minus
    delegate option * to hull except [list -increasekey -descreasekey]
    delegate method * to hull 
    constructor {args} {
        installhull $win
        set textw $win
        $self configurelist $args
        bind $win <$options(-increasekey)>  [mymethod changeFontSize +2]
        bind $win <$options(-decreasekey)>  [mymethod changeFontSize -2]        
    }
    method changeFontSize {x} {
        set font [$win cget -font]
        font configure $font -size [expr {[font configure $font -size] + $x}]
    }
    onconfigure -increasekey value {
        if {$value ne ""} {
            bind $win <$value>  [mymethod changeFontSize +2]
            set options(-increasekey) $value
        } 
        return $options(-foldkey)
    }
    onconfigure -decreasekey value {
        if {$value ne ""} {
            bind $win <$value>  [mymethod changeFontSize -2]
            set options(-decreasekey) $value
        } 
        return $options(-decreasekey)
    }
}
...
#' 
#' <a name="txtabspace">**dgw::txtabspace**</a> *[tk::text pathName] ?-option value ...?*
#' 
#' Creates and configures the *dgw::txtabspace*  widget using the Tk window id _pathName_ and 
#' the given *options*. All options are delegated to the standard text widget. 
#' This creates a text widget where the tab key inserts four spaces but not a tabstop, to enter
#' a TAB you have to press the F2 key.
#' 
#' Methods: None
#' 
#' TODO: Shift-Tab to unindent, TAB to indent a complete region
#'
#' Example:
#' 
#' > ```
#' # demo: txtabspace
#' dgw::txtabspace [tk::text .txt]
#' foreach col [list A B C] { 
#'    .txt ins  end "# Header $col\n\nSome text\n\n"
#' }
#' .txt insert end "Press a TAB key to see the insertion of four spaces."
#' .txt insert end "Press a F2 key to see the insertion of a tab stop."
#' pack .txt -side top -fill both -expand yes
#' > ```
# }
#' 

snit::widgetadaptor dgw::txtabspace {
    delegate option * to hull
    delegate method * to hull
    constructor {args} {
        installhull $win
...
if {[info exists argv0] && $argv0 eq [info script] && [regexp {txmixins} $argv0]} {
    ...
    if {[llength $argv] == 1 && [lindex $argv 0] eq "--version"} {    
        ...
        puts "    -------------------------------------\n"
        puts "Copyright (c) 2021-2022 Dr. Detlef Groth, E-mail: detlef(at)dgroth(dot)de\n"
        ...
        puts "        --demo  marker : runs a specific example from the manual such as txfold"
