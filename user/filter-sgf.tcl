...
#' format. Binaries of The application can be downloaded for Windows, MacOS and Linux from the Github project page at
...
#' ;B[pd];W[dp];B[cd];W[qp];B[op];W[oq];B[nq];W[pq];B[cn];W[fq]LB[jj:A];B[mp];W[qn]
...
#' ```
#' 
#' Using the option `{.sgf eval=true results="hide"}` the output of the command line 
#' tool can be hidden. If you would to use the last added sgf code within the next source
#' code block, just enter the string LASTSGF in the current code block.
#' 
#' ```{.sgf eval=true results="hide" width=500}
#' LASTSGF
#' ```
#' 
#' There is as well an option to select the style, to just use black and white you have to
#' use the argument style=minimalist like here:
#'
#' ```
...
#' ```
#' 
#' The main code chunk options which are translated into command line arguments are shown above.
#' However sometimes you would lile to use other command line options which are not supported by the filter. For this you can use the _args_ code chunk option. Let's say we would like to just display the first 10 moves using the -n option:
#'
#' ```{.sgf eval=true results="hide" width=500 style=minimalist args="--move-numbers=1 -n 10"}
#' LASTSGF
#' ```
#'
#' Let's now display the next ten moves:
#'
#' ```{.sgf eval=true results="hide" width=500 style=minimalist args="--move-numbers=11 -n 20"}
#' LASTSGF
#' ```

#'
#' Here an example for a 9x9 board with some added annotations:
#'
#' ```
...
#' ```
#'
#' To hide the code chunk you should use the option `echo=false`, as you see below only the board but not the code is displayed.
#'
#' 
#' ```{.sgf eval=true results="hide" width=300 style=fancy movenumbers=1 echo=FALSE}
#' LASTSGF
#' ```
#'
#' ## See also:
